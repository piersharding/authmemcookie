...
MAKE = make
FULLEXT = Apache/Auth/AuthMemCookie
BASEEXT = AuthMemCookie
PARENT_NAME = Apache::Auth
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Apache/Auth/AuthMemCookie.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Apache/Auth
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Apache/Auth
...
TO_INST_PM = lib/Apache/Auth/AuthMemCookie.pm

PM_TO_BLIB = lib/Apache/Auth/AuthMemCookie.pm \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/Apache/Auth/AuthMemCookie.pm $(INST_MAN3DIR)/Apache::Auth::AuthMemCookie.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Apache2::Log" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Apache/Auth/AuthMemCookie.pm blib/lib/Apache/Auth/AuthMemCookie.pm 
